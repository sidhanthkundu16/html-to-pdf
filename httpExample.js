...
router.post("/downloadCurrent", (req, res, next) => {
    ...
    let baseheaderPath = path.resolve(
        __dirname,
        "../pdfHeaderTemplate.handlebars"
    );
    let basefooterPath = path.resolve(
        __dirname,
        "../pdfFooterTemplate.handlebars"
    );
    ...
    xml2js.parseString(xml, function (err, result) {
        ...
        try {
            (async () => {
                ...
                let returnedFile = await html_to_pdf({
                    templateHtml,
                    dataBinding,
                    options,
                });

                const file = `data:application/pdf;base64, ${Buffer.from(
                    returnedFile
                ).toString("base64")}`;
